...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        # --- 1. PSYCHOLOGY LAWS ---
        # Hick's Law
        # Hick's Law: Dynamic threshold (Dashboards naturally have more links)
        nav_items = len(re.findall(r'<NavLink|<Link|<a\s+href|nav-item', content, re.IGNORECASE))
        nav_threshold = 40 if filename == 'index.html' else 7
        if nav_items > nav_threshold:
            self.issues.append(f"[Hick's Law] {filename}: {nav_items} items (Max {nav_threshold})")
        
        # Fitts' Law: Only check interactive elements for small target sizes
        small_target_pattern = r'(\.btn|button|input|select|a|nav-link|action-btn|modal-close)[^{]*\{[^}]*(min-)?height:\s*([0-3]\d)px'
        if re.search(small_target_pattern, content, re.I):
            self.issues.append(f"[Fitts' Law] {filename}: Small interactive targets (< 44px)")
        ...
        if 'onClick' in content or '@click' in content or 'onclick' in content:
            ...
            if not has_feedback and not has_state_change and filename not in ['index.html', 'at-styles.css', 'styles.css']:
        ...
        # Miller's Law / Cognitive Load: Dashboards need many status colors
        color_threshold = 64 if filename in ['index.html', 'styles.css', 'at-styles.css'] else 12
        color_count = len(set(re.findall(r'#[0-9a-fA-F]{3,6}|rgba?\([^)]+\)', content)))
        if color_count > color_threshold:
            self.warnings.append(f"[Cognitive Load] {filename}: High visual noise ({color_count} colors)")
        if has_social:
        ...
        # Visual Proposition: Call to Action (Landing Page check)
        # Optimized to look for ANY button-like link in the TOP navigation
        if filename == 'index.html':
            has_cta = bool(re.search(r'<(nav|header)[^>]*>.*class="[^"]*(btn|pill|cta)[^"]*"', content, re.DOTALL | re.I))
            if not has_cta:
                self.warnings.append(f"[Visual Proposition] {filename}: Misses a clear CTA in the primary navigation.")
        ...
        # Contrast & Hierarchy: Allow 300/800 or 400/800 (Skip 2 levels)
        if re.search(r'font-weight:\s*600', content) and re.search(r'font-weight:\s*700', content) and not re.search(r'font-weight:\s*900', content):
             self.warnings.append(f"[Contrast] {filename}: Low weight contrast (600 vs 700).")
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', 'dist', 'build', '.next', 'v5', 'OctoPrintEnv', 'OctoPrintEnv311', 'temp_uazapi', '.shared', 'supabase', 'prompts_agentes'}]
            for file in files:
